export default function solution(input: string) {
	...
	for (const hand of ins) {
		...
		if (cards["J"]) {
			const amountOfJacks = cards["J"];
			delete cards["J"];
			const most = Object.values(cards).max();
			const highestMostOccuring = strength.find(v => cards[v] === most)!;
			cards[highestMostOccuring] = cards[highestMostOccuring] ? cards[highestMostOccuring] + amountOfJacks : amountOfJacks;
		}
